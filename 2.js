...
// 1
// The value of each row is the difference between it's highest and it's lowest number.

diffs = inputArray.map(nums => {
...
// Solution
console.log(diffs.reduce((acc, current) => acc += current))

// 2
// For each row, find the numbers where one evenly devides the other.
// The value of the row is the devision of those two.
const rowValues = (arr) => arr.map(row => {
  let rowValue = 0;
  for (let i = 0; i < row.length; i++) {
    let num = row[i];
    let value;
    row.find(compareNum => {
      if (compareNum === num) return false
      let mod = num % compareNum
      // console.log(num, compareNum, mod)
      if (mod === 0) {
        value = num / compareNum
        return true
      }
    })
    if (value) {
...
// console.log(rowValues([[2, 3, 4]])[0] === 2)
// console.log(rowValues([[5, 9, 2, 8]])[0] === 4)
// console.log(rowValues([[9, 4, 7, 3]])[0] === 3)

// Solution
console.log(rowValues(inputArray).reduce((acc, current) => acc += current))
