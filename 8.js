...
const instructions = input.split('\n').map(row => {
  ...
  return {
    ...
    condition: {
      refId: parts[4],
      compareParam: parts[5],
      compareValue: parseInt(parts[6], 10)
    }
  }
})
...
instructions.forEach(instruction => {
  const { condition, id, change } = instruction
  // Find the new value to write
  const base = values[condition.refId] || 0
...
const finalValues = Object.keys(values).map(key => values[key])
console.log(Math.max(...finalValues))
